...
{#if data.is_unlock}
    <small>
        统计周期{new Date(Number(data.start_time) * 1000).toLocaleDateString()} -
        {new Date(Number(data.end_time) * 1000).toLocaleDateString()}
    </small>

    <Tile style="background-color:#1d2538;border-radius:1em;padding:1em 0">
        <div style="color:#d6c099;text-align:center;font-size:x-large">
            挑战回顾
        </div>
        ...
        <div class="group outstand">
            <span>
                ...
                {#each data.defeat_rank as char}
                    ...
                    <img src={char.avatar_icon} class="icon" alt={char.value} />
                {/each}
            </span>
            <span>
                <small>最强一击</small>
    ...
    {#each data.floors.reverse() as floor}
        ...
        >
            <div slot="above" style="padding:1em">
                ...
                >
                <span style="float:right">⚛ {floor.star}/{floor.max_star}</span>
            </div>
